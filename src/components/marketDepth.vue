    ...
    <div class="market-success">
        ...
        >
            <article
                ...
                :key="index"
            >
                <span class="success-time">{{ convertTime(value.ftime) }}</span>
...
<script>
import { api_orderBlotter,COMPANYNAME,WEBSOCKETBASE } from "../api/api";
export default {
  name: "MarketDepth",
  data() {
    return {
  ...
  watch: {
    '$route'(to, from) {
      ...
      ws = new WebSocket(WEBSOCKETBASE + COMPANYNAME + "/" + product);
  ...
  methods: {
    convertTime(ftime) {
      let ti = new Date(ftime);
      return ti.getHours() + ":" + ti.getMinutes() + ":" + ti.getSeconds();
    },
    orderBlotter() {
      // let product = "Gold Swaps";
      let product = this.$route.params.product;
      api_orderBlotter(product)
        .then(data => {
  ...
  mounted() {
    this.connect();
    window.setInterval(this.orderBlotter, 1000);
    // this.$route.go(0);
  }
};
</script>
